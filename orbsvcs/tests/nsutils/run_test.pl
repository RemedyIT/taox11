...
$LS = $server2->CreateProcess ("$ENV{TAOX11_ROOT}/bin/tao_nslist", "");
$AD = $server3->CreateProcess ("$ENV{TAOX11_ROOT}/bin/tao_nsadd", "");
$DL = $server4->CreateProcess ("$ENV{TAOX11_ROOT}/bin/tao_nsdel", "");
...
sub name_server
{
    $NS->Arguments("-o $server1_naming_ior -m 0 -ORBDebugLevel 0 @_");
#     $NS->Arguments("-o $server1_naming_ior -m 0 @_");
    ...
    print "Naming server is running\n";
...
sub nslist
{
    $LS->Arguments("-ORBDebugLevel 0 -ORBInitRef NameService=file://$server2_naming_ior @_");
