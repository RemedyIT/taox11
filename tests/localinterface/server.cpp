...
main(int argc, ACE_TCHAR *argv[])
{
  ...
  try
  {
    ...
    if (!obj)
    {
      TAOX11_TEST_ERROR << "ERROR: resolve_initial_references (\"RootPOA\") "
    ...
    TAOX11_TEST_DEBUG << "create Hello servant" << std::endl;
    CORBA::servant_traits<Test::Hello>::ref_type hello_impl =
      CORBA::make_reference<Hello> (orb, root_poa, result);
    ...
    {
      TAOX11_TEST_ERROR << "ERROR: root_poa->id_to_reference (id) returned null reference."
        << std::endl;
      return 1;
    }
    ...
    if (hello == nullptr)
    {
      TAOX11_TEST_ERROR << "ERROR: IDL::traits<Test::Hello>::narrow (hello_obj) returned "
        << "null reference." << std::endl;
      return 1;
    }

    const std::string &ior = orb->object_to_string(hello);
