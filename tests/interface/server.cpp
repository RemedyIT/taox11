  ...
  try
    {
      ...
      CORBA::servant_traits<Test::iFactory>::ref_type hello_impl =
          CORBA::make_reference<iFactory_impl> (_orb, root_poa);

      TAOX11_TEST_DEBUG << "created iFactory servant" << std::endl;

      PortableServer::ObjectId id = root_poa->activate_object (hello_impl);

      TAOX11_TEST_DEBUG << "activated iFactory servant" << std::endl;
      ...
      Test::iFactory::_ref_type hello = IDL::traits<Test::iFactory>::narrow (hello_obj);

      if (hello == nullptr)
      {
        TAOX11_TEST_ERROR << "ERROR: IDL::traits<Test::iFactory>::narrow (hello_obj) returned null reference." << std::endl;
        return 1;
      }
      ...
      TAOX11_TEST_DEBUG << "IOR for iFactory servant written to 'test.ior' : " << ior << std::endl;
