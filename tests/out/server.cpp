  ...
  try
    {
      ...
      CORBA::servant_traits<Test::Hello_Factory>::ref_type hello_factory_impl =
          CORBA::make_reference<Hello_Factory> (_orb, root_poa);

      TAOX11_TEST_DEBUG << "created Hello_Factory servant" << std::endl;

      PortableServer::ObjectId id = root_poa->activate_object (hello_factory_impl);

      TAOX11_TEST_DEBUG << "activated Hello_Factory servant" << std::endl;

      IDL::traits<CORBA::Object>::ref_type hello_factory_obj = root_poa->id_to_reference (id);

      if (hello_factory_obj == nullptr)
      {
