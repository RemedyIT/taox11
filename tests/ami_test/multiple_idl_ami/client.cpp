...
public:
  ...
  void op_A (int32_t ami_return_val) override
  {
    ...
    if (ami_return_val != 109)
      {
        TAOX11_TEST_ERROR << "ERROR: ami_return_val not 109: "
                    << ami_return_val << std::endl;
        result = 1;
      }
    cb_a++;
...
public:
  ...
  void op_B (int32_t ami_return_val) override
  {
    TAOX11_TEST_INFO << "Callback method <BHandler::op_B> called: ret "
                << ami_return_val << std::endl;
    if (ami_return_val != 108)
      {
        TAOX11_TEST_ERROR << "ERROR: Callback method <BHandler::op_B> ami_return_val not 108: "
                    << ami_return_val << std::endl;
        result = 1;
      }
    cb_b++;
  ...
  try
    {
      ...
      TAOX11_TEST_INFO << "Client: Sending asynch messages A::op_A, B::op_B." << std::endl;

      test_async_b->sendc_op_B ( test_handler_b, 8);
