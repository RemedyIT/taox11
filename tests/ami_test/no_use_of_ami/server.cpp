  ...
  try
    {
      ...
      CORBA::servant_traits<Test::B>::ref_type hello_impl =
        CORBA::make_reference<B_i> (_orb);


      TAOX11_TEST_INFO << "created B servant" << std::endl;

      PortableServer::ObjectId id = root_poa->activate_object (hello_impl);

      TAOX11_TEST_INFO << "activated B servant" << std::endl;
      ...
      IDL::traits<Test::B>::ref_type hello =
          IDL::traits<Test::B>::narrow (hello_obj);

      if (hello == nullptr)
        {
          TAOX11_TEST_ERROR
            << "ERROR: IDL::traits<Test::B>::narrow (hello_obj) returned null reference."
            << std::endl;
          return 1;
        }
      ...
      TAOX11_TEST_INFO << "IOR for B servant written to 'server.ior' : " << std::endl;
