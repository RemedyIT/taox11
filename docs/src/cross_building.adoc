...
install and run all tooling. Currently TAOX11 (AXCIOMA) only supports mainstream Linux desktop
environments (OpenSuSE, Debian, Fedora, CentOS, RHEL) as host environment for cross building.
...
in the cross build toolchain's sysroot like the RTI NDDS version for your target, a Xerces-C library version
...
Currently TAOX11 (AXCIOMA) provide the following standard target speficiation file:
