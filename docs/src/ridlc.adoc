...
Any IDL4 support is experimental and not usable for production usage.
The IDL to {cxx11} language does provide a mapping for the IDL4 extended datatypes, but
but only for DDS there is a wire protocol for transferring these types through the wire.
For CORBA/LwCCM a lot of parts are not standardized yet and need still a lot of
