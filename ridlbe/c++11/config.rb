...
module IDL
  module Cxx11
    ...
    def self.add_extended_options(ol, _idl_params)
      ...
      ol.for_switch '-G{generation options}', type: String, separator: true do |swcfg|
        ...
        swcfg.define_group :gen_flags, params: {
              ...
              'xhcp' => { option_name: :gen_export_client_proxy,
              ...
              'cpc' => { option_name: :gen_client_proxy_source,
      ...
      ol.for_switch '-S{suppression options}', type: String, separator: true do |swcfg|
        swcfg.define_group :suppress_flags, params: {
              ...
              'cph' => { option_name: :no_client_proxy_hdr,
    ...
    def self.check_client_proxy_export_params(options)
      if options.gen_export_client_proxy || options.export_client_proxy
        unless options.client_proxy_export_macro || options.base_export_macro
          IDL.fatal("ERROR: it isn't allowed to use -Gxhcp or -Xcp without specifying the macro with -Wb,client_proxy_export_macro=MACRO " +
                        'or with -Wb,base_export_macro=MACRO_PREFIX')
        end
        # only in case export header generation has been explicitly enabled will
        # we derive missing export parameters from base parameters
    ...
    def self.generate_client_stubs(options)
      ...
      if options[:gen_client_proxy_source]
