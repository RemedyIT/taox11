  ...
  module Cxx11
    ...
    def self.add_extended_options(ol, _idl_params)
        ...
        swcfg.for_group :b_extopt do |grpcfg|
          ...
          grpcfg.for_params :strings,
            params: {
              ...
              'stub_proxy_export_macro' => { description: "-Wb,stub_proxy_export_macro=MACRO\tSet export macro for client proxy files" },
              'stub_proxy_export_include' => { description: "-Wb,stub_proxy_export_include=FILE\tSet export include file for client proxy files" },
      ...
      ol.for_switch '-G{generation options}', type: String, separator: true do |swcfg|
        ...
        swcfg.define_group :gen_flags, params: {
              ...
              'atc' => { option_name: :gen_anytypecode_source,
                       description: "-Gatc\t\t\tGenerate Any and TypeCode support in separate *A.cpp file  (not generated by default)" },
              'ath' => { option_name: :gen_anytypecode_header,
                       description: "-Gath\t\t\tGenerate Any and TypeCode support in separate *A.h file  (not generated by default)" },
              ...
              'uia' => { option_name: :gen_union_invalid_argument,
                       description: "-Guia\t\t\tGenerate std::invalid_argument for union API misusage (CORBA::BAD_PARAM by default)" },
              ...
              'xhcp' => { option_name: :gen_export_stub_proxy,
                       description: "-Gxhcp\t\t\tGenerate export header file for client proxy (not generated by default)" },
              'xhat' => { option_name: :gen_export_anytypecode,
                       description: "-Gxhat\t\t\tGenerate export header file for Any and TypeCode support (not generated by default)" },
              ...
              'cpc' => { option_name: :gen_stub_proxy_source,
                       description: "-Gcpc\t\t\tGenerate implementation of client proxy is separate *CP.cpp file (not generated by default)" },
            }
      end
      ...
      ol.for_switch '-X{export options}', type: String, separator: true do |swcfg|
        swcfg.define_group :export_flags, params: {
              ...
              'cp' => { option_name: :export_stub_proxy,
      ...
      ol.for_switch '-S{suppression options}', type: String, separator: true do |swcfg|
        swcfg.define_group :suppress_flags, params: {
              ...
              'cph' => { option_name: :no_stub_proxy_hdr,
                       description: "-Scph\t\t\tSuppress generation of client proxy header file (generated by default)" },
    ...
    Backend.configure('c++11', File.dirname(__FILE__), TITLE, COPYRIGHT, IDL::Cxx11.taox11_version) do |becfg|
      ...
      becfg.on_process_input do |parser, options|
        ...
        IDL::Cxx11.check_stub_proxy_export_params(options)
        ...
        if options[:gen_export_stub_proxy]
          IDL::Cxx11.gen_stub_proxy_export(options)
        end
    ...
    def self.setup_output(options)
      ...
      unless options[:idlfile].nil?
        ...
        options[:output_stub_proxy_header] = File.join(options[:outputdir], File.basename(options[:idlfile], idl_ext) + options[:stub_pfx] + options[:proxy_pfx] + options[:hdr_ext])
        options[:output_stub_proxy_source] = File.join(options[:outputdir], File.basename(options[:idlfile], idl_ext) + options[:stub_pfx] + options[:proxy_pfx] + options[:src_ext])
        options[:output] << options[:hdr_ext]
        if options[:gen_typecodes] && options[:gen_anytypecode_source]
          options[:output_anytypecode_source] = File.join(options[:outputdir], File.basename(options[:idlfile], idl_ext) + options[:anytypecode_pfx] + options[:src_ext])
        end
        if options[:gen_typecodes] && options[:gen_anytypecode_header]
          options[:output_anytypecode_header] = File.join(options[:outputdir], File.basename(options[:idlfile], idl_ext) + options[:anytypecode_pfx] + options[:hdr_ext])
        end
      end
    end
    ...
    def self.check_stub_proxy_export_params(options)
      if options.gen_export_stub_proxy || options.export_stub_proxy
        unless options.stub_proxy_export_macro || options.base_export_macro
          IDL.fatal("ERROR: it isn't allowed to use -Gxhcp or -Xcp without specifying the macro with -Wb,stub_proxy_export_macro=MACRO " +
                        'or with -Wb,base_export_macro=MACRO_PREFIX')
        end
        # only in case export header generation has been explicitly enabled will
        # we derive missing export parameters from base parameters
        options.stub_proxy_export_macro = options.base_export_macro + '_CLIENT_PROXY' + options.export_macro_pfx unless options.stub_proxy_export_macro || options.base_export_macro.nil?
        options.stub_proxy_export_include = options.base_export_include + '_stub_proxy' + options.export_header_pfx unless options.stub_proxy_export_include || options.base_export_include.nil?
      end
    end
    ...
    def self.generate_client_stubs(options)
      ...
      unless options[:no_stub_proxy_hdr]
        if options[:output_stub_proxy_header]
          co_prx = GenFile.new(options[:output_stub_proxy_header])
          IDL.push_production(:stub_proxy_header, ::IDL::Cxx11::StubProxyHeaderWriter.new(co_prx, options))
        end
      end
      if options[:gen_stub_proxy_source]
        if options[:output_stub_proxy_source]
          co_prx = GenFile.new(options[:output_stub_proxy_source])
    ...
    def self.generate_ami_support(options)
      if options[:client_stubs]
        ...
        if options[:ami_stub_output_prx]
          ...
          IDL.push_production(:ami_stub_proxy_header, ::IDL::Cxx11::AmiStubProxyHeaderWriter.new(co_prx, options))
        end
      end
    end
    ...
    def self.gen_stub_proxy_export(options)
      return if IDL.has_production?(:stub_proxy_export_header)

      export_file = options.stub_proxy_export_file || options.stub_proxy_export_include
      if export_file
        so = GenFile.new(File.join(options.outputdir, export_file))
        IDL.push_production(
            :stub_proxy_export_header,
            ::IDL::Cxx11::ExportHeaderWriter.new(options.stub_proxy_export_macro, export_file, so, options))
      else
        IDL.fatal('ERROR: it is not allowed to use -Gxhimpl without specifying the file with -Wb,impl_export_file=FILE, ' +
                      '-Wb,impl_export_include=FILE or -Wb,base_export_include=FILE_PREFIX')
      end
